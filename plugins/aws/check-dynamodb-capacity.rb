...
#
# USAGE:
#   Warning if a table's consumed rw capacity is over 80%, critical if over 90%
#   check-dynamodb-capacity --warning-over 80 --critical-over 90
#
#   Critical if session table's consumed read capacity is >90%,
#   maximum of last 1 hour
#   check-dynamodb-capacity --table_names session --capacity-for read \
#   --critical-over 90 --statistics maximum --period 3600
#
# NOTES:
#
# LICENSE:
...
class CheckDynamoDB < Sensu::Plugin::Check::CLI
  ...
  %w(warning critical).each do |severity|
    option :"#{severity}_over",
           ...
           proc:        proc(&:to_f),
           description: "Trigger a #{severity} if consumed \
           capacity is over a percentage"
  end

  def aws_config
    hash = {}
    hash.update access_key_id: config[:access_key_id], secret_access_key: \
    config[:secret_access_key] if config[:access_key_id] \
  ...
  def tables
    ...
    @tables.select! { |table| config[:table_names].include? table.name } \
    if config[:table_names]
    @tables
  end

  def cloud_watch_metric(metric_name, table_name)
    cloud_watch.metrics.with_namespace('AWS/DynamoDB')\
    .with_metric_name(metric_name).with_dimensions(name: 'TableName',
                                                   value: table_name).first
  end
  ...
  def latest_value(metric)
    metric.statistics(statistics_options.merge unit: 'Count')\
    .datapoints.sort_by { |datapoint| datapoint[:timestamp] }\
  ...
  def check_capacity(table)
    config[:capacity_for].each do |r_or_w|
      ...
      percentage    = metric_value / table.send("#{r_or_w}_capacity_units")\
      .to_f * 100
      ...
      @severities.keys.each do |severity|
        ...
        flag_alert severity, "; On table #{table.name} consumed #{r_or_w} \
        capacity is #{sprintf '%.2f', percentage}% \
        (expected_lower_than #{threshold})"
        break
      end
    end
  end
  ...
  def run
    ...
    @message += "; (#{config[:statistics].to_s.capitalize} \
    within #{config[:period]} seconds "
    @message += "between #{config[:end_time] - config[:period]} \
    to #{config[:end_time]})"
